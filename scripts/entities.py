...
def get_reason():
    paragraph_dict = {}
    file_names = [file_name for file_name in os.listdir(raw_DIR) if '.html' in file_name] 
...
def send_to_refinitiv(paragraph_dict):
    ...
    for name,paragraph in paragraph_dict.items(): #iterate over dictionary, separate between keys and values
        try:
